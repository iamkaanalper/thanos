...
- `friction_checklist_brief`: `""` — Faz 2: Ledger'dan gelen yüksek friction pattern'lerine göre dinamik checklist (friction_checklists.py ile hazırlandı). Reviewer prompt'una enjekte edilir.
...
### Faz 2 — Friction Checklist Briefing (Ledger'dan)

Setup'tan hemen sonra (Step 1'den önce veya hemen sonra), orchestrator şu komutu çalıştırarak `friction_checklist_brief`'i hazırlar:
...
Çıktıyı `friction_checklist_brief` state'ine kaydet. Boş çıkarsa sessizce geç.

Bu briefing, Step 2'deki reviewer prompt'una enjekte edilir (aşağıda).
...
**Hook + Compound Learning Entegrasyonu**

Review bittikten sonra hook sistemini tetikle (yeni disiplin):

```python
from grok.hooks.core.hook_runner import run_hook

run_hook(
    "on_verifier_run",   # veya on_friction_detected eğer kritik bulgu varsa
    verdict="PASS" if total_issues == 0 else "FAIL",
    critical_issues=[i for i in issues if i.severity == "bug"],
    important_issues=issues,
    session_context=f"review {REVIEW_ID}",
)
```

**Faz 2 — Compound Learning Entegrasyonu (A: Review → Compound Köprüsü):**

Bu review, compound-learnings self-improvement sisteminin önemli bir veri kaynağıdır.

Review tamamlandıktan sonra orchestrator şu adımları **otomatik** olarak yapar:

1. Eğer review'da anlamlı friction tespit edildiyse (3+ bug, 1+ security bulgusu, veya toplam 5+ issue), compound-learnings analyzer'ını hafif modda çalıştırır (Faz 2 - B: review moduna özel):
   ```bash
   python3 ~/.grok/skills/compound-learnings/scripts/analyze.py --min 2 --source review
   ```

2. Analyzer çıktısından yüksek friction yaratan pattern'ler çıkarılır ve şu iki yere beslenir:
   - compound-learnings öneri sistemine (mevcut davranış)
   - Bu review'un özet dosyasına yapılandırılmış "Compound Suggested Patterns" bölümü olarak

3. Final Report'a şu bilgiler eklenir:
   - Compound Analysis tetiklendiği bilgisi
   - Tespit edilen yüksek friction pattern'lerin özeti
   - Bu pattern'lerin gelecekteki çalışmalarda hatırlatılacağı notu

Bu adım, review sonuçlarını compound öğrenme döngüsüne **doğrudan ve kalıcı etki yaratacak şekilde** dahil etmenin ilk somut köprüsüdür (Faz 2 - A).
...
## Faz 1 Orchestration Disiplini (Role Assignment + QA-Loop + Handoff)

Bu skill, `docs/role-assignment.md` (v2) içindeki karar matrisine ve Bounded QA-Loop kurallarına uymakla yükümlüdür.

- **Security-sensitive değişikliklerde** (auth, secrets, payment, yeni endpoint, data handling): Reviewer persona'sı **tek başına** yeterli değildir. Orchestrator veya kullanıcı, `security-auditor` persona'lı ayrı bir review çalıştırmalıdır.
- **Handoff disiplini**: Bu skill bağımsız review için kullanıldığında bile, bulgular mümkün olduğunca `handoff` skill'indeki QA PASS/ISSUES formatına yakın yazılmalıdır (özellikle PR modunda GitHub'a gönderilen yorumlar).
- **Self-Improvement Flywheel**: Reviewer persona'sı artık "Self-Improvement Flywheel Participation" maddesi içerir. Bulgular genelleştirilebilir şekilde yazılmalıdır. Bu review sonuçları (özellikle tekrar eden pattern'ler) compound-learnings sistemine beslenir.

## Faz 2 — Dinamik Friction Odak (Ledger Kategorilerine Göre) — Uygulandı

Orchestrator artık `~/.grok/compound-friction.jsonl` dosyasını okuyup High/Medium impact pattern'leri `friction_checklists.py` modülüne verir ve `get_checklist_for_ledger_patterns(...)` ile dinamik checklist üretir.

Bu checklist, reviewer prompt'una **otomatik olarak enjekte edilir** (yukarıdaki "Faz 2 — Friction Checklist Briefing" ve Step 2).

Reviewer artık şu kurala uyar:
- Checklist bloğu prompt'ta varsa, o kategorilerdeki sorunlara **öncelikli ve daha derin** odaklan.
- Bulguları yazarken checklist maddeleriyle ilgili olanları özellikle genelleştir (compound flywheel için kaliteli sinyal).

Bu, review skill'ini implement ve execute-plan ile aynı seviyede friction-aware yapar.

Eski metin (tarihi kayıt için):
Eğer `~/.grok/compound-friction.jsonl` dosyasındaki son kayıtlar belirli kategorilerde yoğunlaşıyorsa, reviewer şu ekstra odaklara dikkat etmelidir:

- Handoff / Context eksikliği kategorisi ağır basıyorsa → her boundary'de handoff kalitesini ve context aktarımını özellikle incele.
- Factcheck-Guard / "okumadan claim" kategorisi ağır basıyorsa → her bulguda "okunmuş mu?" kontrolünü daha sıkı yap.
- Security / validation / error handling kategorileri ağır basıyorsa → bu alanları daha derin ve öncelikli incele.

Bu odaklar, compound-learnings ledger'ından otomatik olarak türetilir ve review kalitesini geçmiş friction'lara göre optimize eder.

**Daha yapılandırılmış checklist için** şu modülü kullan:
`~/.grok/skills/compound-learnings/scripts/friction_checklists.py`
→ `get_checklist_for_ledger_patterns(patterns)` fonksiyonu ile doğrudan checklist metni alabilirsin.
...
## Task Lifecycle Ledger Entegrasyonu — Production Contract (Review Skill)

Bu bölüm, review skill'ini implement ile aynı seviyede ledger-aware yapar. Review, özellikle `/implement --effort N` içinden çağrıldığında multi-round Dev-QA döngüsünün kritik parçasıdır. Bu durumda reviewer'ın gerçek attempt state'ini görmesi zorunludur.

### Kullanım Kuralı (Review Bağlamında)

| Durum | Ledger Context Enjekte Edilmeli mi? | Gerekçe |
|-------|-------------------------------------|---------|
| Bağımsız `/review` (kullanıcı direkt çağırdı) | Opsiyonel (tavsiye) | Tek shot review |
| `/implement` içinden reviewer olarak çağrıldığında (herhangi bir round) | **Zorunlu** | attempt + accumulated_feedback + escalation state gerçek olmalı |
| 2. veya 3. review round (re-review) | **Zorunlu** | Reviewer önceki feedback'i tekrar etmemeli, sadece yeni sorunlara odaklanmalı |
| PR mode'da bile olsa implement döngüsü içindeyse | **Zorunlu** | GitHub'a gönderilen yorumlar bile ledger state'ini yansıtmalı |

**Kural:** Eğer bu review çağrısı bir `TaskLifecycleLedger` state'i olan bir implement run'ından geliyorsa, `make_devqa_handoff_context` sonucu reviewer prompt'una **verbatim** enjekte edilmelidir.

### Concrete Wiring (Orchestrator Tarafında)

```python
from bundled.skills.shared.task_lifecycle import make_devqa_handoff_context

# Reviewer başlatmadan hemen önce (implement orchestrator içinde)
ledger_context = make_devqa_handoff_context(ledger, IMPL_ID)

# Reviewer prompt'una ekle
reviewer_prompt += "\n\n" + ledger_context.get("task_lifecycle", "")
```

### Faydalar

- Reviewer gerçek "attempt N/3" ve birikimli feedback görür.
- Aynı sorunu tekrar rapor etme riski ortadan kalkar.
- Escalation kararları veri odaklı olur.
- Compound learnings için kaliteli, versiyonlu sinyal.

**Bugün yapılan:** Dosyanın sonunda ~15 kez tekrarlanan kötü blok tamamen temizlendi. Yerine tek, yetkin Production Contract bölümü kondu.

Artık review skill'i de implement ile aynı disiplin seviyesindedir.

---

**Sonraki doğal adım:** Bu entegrasyonu `execute-plan` skill'ine de uygulamak.
