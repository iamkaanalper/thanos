...
## Mandatory Pre-Flight Discipline (Claude Integration)

Before any implementation work begins on non-trivial tasks, the following **must** occur:

1. **Explore First (Read-Only Phase)**
   - If the task touches existing code or is larger than a trivial fix, perform structured exploration first (use `explore` skill if available, or equivalent tldr / codebase analysis).
   - Goal: Understand structure, patterns, entry points, and risks *before* writing or spawning implementers.
   - Output: Either a short exploration summary or handoff artifact.

2. **Use Structured Handoffs for All Subagent Launches**
   - Every `spawn_subagent` prompt for implementation, review, or fix work **must** include (or reference) a handoff from the `handoff` skill.
   - Primary templates: "Standard Task Handoff", "QA / Review Handoff (PASS or FAIL)", "Diagnosis Report", "Escalation".
   - Never launch a subagent with only vague prose. Context loss is the enemy.

3. **Reference the Handoff Skill**
   - The handoff templates live at: `C:\Users\kaana\.grok\skills\handoff\SKILL.md`
   - When in doubt, read that file at the start of a complex run and use its formats.

These rules come from the highest-ROI patterns in the mature Claude/the original Claude Code AI software team system skill set (qa-loop, handoff-templates, explore, build). They are non-negotiable for work that matters.
...
- `compound_analysis_output`: `""` — full stdout captured from `analyze.py --min 3 --draft` after memory flush. Embedded verbatim into the Final Report under "Compound Learnings & Self-Improvement Proposals".
- `compound_drafts_generated`: `[]` — paths to any High-impact draft artifacts written by the analyzer (for permanent record + later --apply).
- `friction_checklist_brief`: `""` — Faz 2: Ledger'dan gelen friction pattern'lerine göre dinamik checklist (friction_checklists.py kullanılarak hazırlandı). Implementer ve reviewer prompt'larına enjekte edilir.

## Specialization Selection

**Bu karar, `docs/role-assignment.md` dosyasındaki "Task → Persona + Effort + Handoff Karar Matrisi"ne göre verilmelidir.**
...
**Önemli Bağlantı:**
- Auth, user input, secrets, payments, yeni endpoint gibi güvenlik hassasiyeti olan işlerde → `security-auditor` **zorunlu** olarak reviewer slot'larından birine eklenir (role-assignment.md matrisine göre).
- Büyük yapısal değişimlerde → reviewer sayısı artırılır.
- Basit işlerde → effort 1 + tek reviewer yeterli olabilir.
...
### Faz 2 — Preflight + Friction Curator (Zorunlu Entegrasyon — Sırayla #2)

Büyük / 2+ round riski taşıyan işlerde **ilk iş** olarak preflight skill'inin gerçek executable implementasyonunu çağır.

```python
# --- Sırayla #2: Gerçek preflight + friction-curator entegrasyonu ---
try:
    ...
    pf = run_preflight(
        ...
        workspace_id=workspace_id,  # memory.py'den gelen
    )
    friction_checklist_brief = pf.get("friction_checklist_brief", "")

    # Opsiyonel: Curator'dan daha olgun, tekrar eden pattern önerileri de ekle (yüksek sinyal)
    curator_block = get_preflight_suggestions_from_curator()
    if curator_block:
        friction_checklist_brief = (friction_checklist_brief + "\n\n" + curator_block).strip()
except Exception:
    friction_checklist_brief = ""
    # Graceful: eski inline mantık hiçbir zaman tamamen kırılamaz
```

Bu, preflight skill'ini (bundled/skills/preflight/SKILL.md) ve friction-curator'u (friction_curator.py) **ilk sınıf** orkestratör çağrısına dönüştürür.

- `run_preflight` hem `on_implement_start` hook'unu güvenli şekilde tetikler hem de high-impact friction pattern'lerini checklist olarak döndürür.
- `get_preflight_suggestions_from_curator` friction-curator'un tekrar eden yüksek-etkili pattern'leri kalıcı Pre-Flight kuralı haline getirme önerilerini verir.

Küçük tek-dosya fix'lerde bu blok atlanabilir (graceful). Herhangi bir 2+ round ihtimali olan işte **atlanması Production Contract ihlalidir**.

Bu `friction_checklist_brief`, implementer ve reviewer prompt'larına enjekte edilir (aşağıda).
...
**Production Contract (mandatory for 2+ round risk):** Use `spawn_with_discipline` (from spawn_helper) for the actual spawn call. This auto-injects ledger + handoff.

First build context if needed:
```python
...
# Assume self.ledger and IMPL_ID set in Setup
ctx = build_spawn_context(
    ledger=self.ledger,
    task_id=IMPL_ID,
    base_prompt=base_implementer_prompt,  # the one with persona + issues + friction
    objective=description
)
full_prompt = ctx["full_prompt"]
```

Then launch (use the helper wrapper for the tool call):
```python
impl_task = spawn_with_discipline(
    ...
    background=False,
    description=f"[implementer] {short}",
    base_prompt=...,  # or pass the full after build
    ledger=self.ledger,
    task_id=IMPL_ID,
    objective=description
)
...
(If the wrapper not directly available in this context, fall back to manual spawn_subagent with the full_prompt that includes the ctx blocks.)

The concrete prompt sent to the subagent must contain the `task_lifecycle` block and handoff from the helper.

(Full ready-to-paste in the "Task Lifecycle Ledger Entegrasyonu — Production Contract" section below.)
```
...
**Production Contract (mandatory):** Use `spawn_with_discipline` for the launch so ledger state + handoff are auto-injected (critical on re-review rounds for correct "attempt N" and accumulated feedback).

Build + launch:
```python
ctx = build_spawn_context(ledger=self.ledger, task_id=IMPL_ID, base_prompt=base_reviewer_prompt, objective=...)
full_prompt = ctx["full_prompt"]

rev_task = spawn_with_discipline(
    spawn_subagent_fn=spawn_subagent,
    subagent_type="general-purpose",
    description=f"[reviewer] Review implementation",
    base_prompt=...,
    ledger=self.ledger,
    task_id=IMPL_ID,
    objective=...
...
(If wrapper not in scope, manual spawn_subagent with the full_prompt containing the ctx blocks from build_spawn_context.)

Manual version (still works but prefer helper):
```
...
<if friction_checklist_brief is defined and non-empty, include the following block verbatim:>
<friction_checklist_brief>
<end if>
...
### Step 3a: Escalate to User (Faz 1 — Bounded QA-Loop Escalation)

Bu adım, `docs/role-assignment.md` içindeki "Deneme 3 basarisiz → ESCALATION" kuralına tam uygun çalışmak zorundadır.

3. round sonunda hâlâ açık issue varsa, orchestrator **otomatik** olarak escalation başlatır. Kullanıcıya şu 5 seçenek net şekilde sunulur (handoff-templates'teki Escalation formatına yakın):

- **Reassign**: Farklı persona/effort seviyesi ile yeniden dene
- **Decompose**: Task'i daha küçük, bağımsız parçalara böl
- **Revise**: Yaklaşımı veya mimariyi değiştir
- **Defer**: Sonraki iterasyona/sprint'e ertele (neden ile birlikte)
- **Accept**: Bilinen sınırlama ile kabul et + dokümante et

Kullanıcı kararı verdikten sonra:
- Kararı implementer'a ilet.
- İlgili issue'ları `Status: fixed` veya `Status: wontfix` (kullanıcı kararıyla) olarak güncelle.
- "Sadece bir tur daha" şeklinde sınırsız uzatma **yapma**.

Bu kural ihlal edilirse, self-improvement flywheel bu durumu olumsuz pattern olarak kaydeder.
...
## Exit Condition (Bounded QA-Loop — Zorunlu)

**Bu döngü, `docs/role-assignment.md` içindeki "QA-Loop Disiplini (Zorunlu)" bölümüne tam uygun çalışmak zorundadır.**

The **only** exit condition is **all reviewers** reporting **0 issues** of any severity in the same round.

**Sıkı Sınırlama (Kesin Uygulanır):**
- Maksimum **3 review-fix round** yapılabilir.
- Round 3 sonunda hâlâ açık issue varsa → **otomatik escalation** tetiklenir (Step 3a).
- "Bir tur daha deneyelim" şeklinde sınırsız uzatma **yasaktır**.
...
**Proactive Self-Improvement Step (after memory flush) — Explicit Capture Protocol:**

This step runs **orchestrator-side** (no subagent). It must be executed after the memory helper returns successfully.

**Hook-powered auto behaviors (default & automatic)**

Büyük run'ların sonunda kritik hook'lar artık **varsayılan olarak otomatik tetikleniyor** (yeni davranış):

```python
from grok.hooks.core.hook_runner import run_hook

# Otomatik tetiklenen hook'lar (her büyük run sonunda)
run_hook("on_run_completion", session_context=..., issue_patterns=..., issues_by_severity=...)
run_hook("on_self_improvement_cycle", session_context=..., issue_patterns=..., issues_by_severity=...)
run_hook("on_compound_analysis_start", session_context=...)   # analyzer tetikleme için
# Swarm full integration (when using swarm orchestrator)
# run_hook("on_swarm_start", ...)
# run_hook("on_swarm_phase", event="bounded_loop"|"escalation"|"completion", phase=..., track_id=...)
# run_hook("on_phase_end", phase=5, ...)
# run_hook("on_draft_applied", ...)  # via compound_apply_feedback after apply
```

Bu hook'lar şu otomatik davranışları tetikler:
- Friction kaydı + analyzer çalıştırma
- Pre-flight için recent friction injection önerisi
- Draft'lar üretildiğinde curation ve feedback döngüsü

Artık "set and forget" self-improvement çok daha güçlü.

**Exact sequence the orchestrator MUST follow:**

1. **Invoke the analyzer via shell and capture everything**
   ```bash
   python3 ~/.grok/skills/compound-learnings/scripts/analyze.py --min 3 --draft
   ```
   - Use the same `run_terminal_cmd` pattern as the memory update (write temp files if needed, but here we can run directly).
   - Capture **both** stdout and stderr.
   - Record the exit code.

2. **Store the raw output (mandatory)**
   - Set `compound_analysis_output = stdout` (even if empty or error).
   - If exit code != 0:
     - Prepend a clear error header to the stored output:
       "COMPOUND ANALYZER FAILED (exit {code})\n{stderr}\n--- stdout below ---\n{stdout}"
     - Log a brief warning to the user.
     - Continue to Final Report (never fail the whole run for this).

3. **Parse draft paths (best-effort)**
   - Scan the captured stdout line-by-line.
   - Any line containing "→ " followed by a path (e.g. "  → /Users/.../.grok/skills-drafts/compound-learnings/2026-...-handoff.md")
     → append the trimmed path to `compound_drafts_generated`.
   - Also accept lines that say "Draft artifacts generated" or "→ " anywhere in the block.
   - Deduplicate and keep order of appearance.

4. **Graceful degradation**
   - If the python command cannot be executed (python not found, script missing, permission error, timeout), set:
     `compound_analysis_output = "COMPOUND ANALYZER UNAVAILABLE: {brief reason}. Self-improvement step skipped for this run."`
     `compound_drafts_generated = []`
   - Never let this step block the Final Report.

5. **Proceed immediately to Final Report**
   - The Final Report (section 9) will render `compound_analysis_output` verbatim inside the "Compound Learnings & Self-Improvement Proposals" block.
   - If `compound_drafts_generated` is non-empty, the report must list them and offer the `--apply` command.

This explicit protocol ensures the self-improvement data is **always** produced by the orchestrator itself using the same tool-calling discipline as the memory flush, and the results survive into the permanent Final Report with zero reliance on prose description alone.
...
9. **Compound Learnings & Self-Improvement Proposals** (permanent record)
   - This section **always** appears in the Final Report.
   - Read the orchestrator state variables `compound_analysis_output` and `compound_drafts_generated` (populated in the Explicit Capture Protocol of Step 6d).
   - Render `compound_analysis_output` **verbatim** (preserving all formatting, banners, and proposal text) under the heading.
   - If `compound_drafts_generated` is non-empty, append a short "Generated drafts" block listing every path, followed by the exact command the user can run later:
     `python3 ~/.grok/skills/compound-learnings/scripts/analyze.py --apply`
   - If the captured output indicates failure or unavailability, still render it (with the error header) so the run record is honest.
   - If the output is empty or just the "no high-signal" message, render that cleanly.
   - This is the Grok-native realization of the Claude "compound learnings / self-improvement flywheel": every serious implementation run leaves behind not only memory patterns but also concrete proposals for improving the orchestrator, personas, and rules themselves.
   - After the full Final Report, if high-impact drafts exist, offer the user the `--apply` option (with the safety guarantees documented in the compound-learnings skill).

This single section makes the entire self-improvement loop visible, reviewable, and permanent — exactly what the user asked for with choice "1".
...
- After memory flush: "Running Explicit Compound Capture Protocol (analyzer + stdout capture + draft path parsing)..." (results will be embedded verbatim in Final Report)
...
- **Personas now participate in the self-improvement flywheel** — the three core personas (implementer, reviewer, security-auditor) contain an explicit "Self-Improvement Flywheel Participation" constraint. When writing prompts, this expectation is already present; the Explicit Compound Capture Protocol will harvest the resulting higher-quality signals.
- **Mandatory Structured Handoff Usage + Spawn Helper (Enforceable Production Contract)** — 
  - Every non-trivial `spawn_subagent` call (implementer, reviewer, security-auditor) **must** include or explicitly reference a handoff from the `handoff` skill **and** (for any flow with 2+ round risk) use `spawn_with_discipline` / `build_spawn_context` from spawn_helper (or manual equivalent injection of current ledger state + structured handoff).
  - Vague prose-only launches are **prohibited**.
  - Orchestrator, spawn çağrısı yapmadan önce elindeki context'in yeterli handoff + ledger kalitesinde olduğundan emin olmakla yükümlüdür.
  - İhlal durumunda: "Handoff/ledger olmadan devam edilemez" uyarısı ver ve spawn'ı gerçekleştirme (veya en azından çok güçlü bir uyarı + gerekçe iste).
...
## Using the Task Lifecycle Ledger (Post-MVE Enhancement)

For any non-trivial run (especially those likely to need 2+ rounds), the orchestrator **should** use the `task_lifecycle` ledger for real state instead of relying only on text in handoffs or memory files.
...
ledger = TaskLifecycleLedger(session_id=workspace_id)
state = ledger.start_or_resume(task_id=IMPL_ID, objective=description, max_attempts=3)

# When spawning implementer or reviewers
context = make_devqa_handoff_context(ledger, IMPL_ID)
# Inject context["task_lifecycle"] into their prompts

# After each review round
ledger.record_attempt(IMPL_ID, feedback=..., issues=[...])

if state.status == "escalated":
    # Follow qa-loop escalation rules
```

**Benefits:**
- Real attempt count + accumulated feedback visible to everyone.
- Automatic escalation at max_attempts.
- Clean, queryable audit trail for post-mortems and compound learning.
- Directly solves the non-atomic 3-retry race condition discovered in Item 2.

**Rule:** If you are about to track "Round N/3" only in markdown or handoff text, use the ledger instead.

See: 
- `bundled/skills/shared/task_lifecycle.py` + `bounded_devqa_with_ledger.py` example
- `bundled/skills/shared/spawn_helper.py` (build_spawn_context + spawn_with_discipline) for automatic handoff + ledger injection on every `spawn_subagent`
- `bundled/skills/shared/preflight.py` and friction helpers for the rest of the Production Contract.

---

## Task Lifecycle Ledger Entegrasyonu — Production Contract (Zorunlu Yüzey)

Bu bölüm, "Using the Task Lifecycle Ledger" tavsiyesini **gerçek orchestrator akışına** dönüştürür. Bugün (kullanıcı "bu işi bugün biteceğiz" dediği gün) ledger, metin tabanlı "deneme N/3" mantığının yerini alacak şekilde implementasyonun kritik noktalarına bağlandı.

### Neden Bu Gerekliydi
Post-MVE Item 2 sırasında keşfedilen kök neden: qa-loop disiplini (3 retry + birikimli feedback + escalation) sadece prompt metninde ve handoff template'lerinde yaşıyordu. Bu non-atomic bir race condition yaratıyordu (subagent'lar aynı metni farklı okuyabiliyordu). `TaskLifecycleLedger` bu durumu executable state + atomic-ish append-only JSONL ile çözer.

### Kullanım Kuralı (Decision Tree)

| Durum | Ledger Kullan? | Gerekçe |
|-------|----------------|---------|
| Trivial tek-dosya fix, 1 round bekleniyor | Opsiyonel (tavsiye edilmez) | Overhead yok |
| Herhangi bir iş 2+ round ihtimali taşıyorsa (yeni logic, auth, refactor, multi-file) | **Zorunlu** | attempt count + accumulated_feedback + auto-escalation gerçek state'ten gelir |
| Paralel reviewer'lar veya worktree isolation kullanıyorsan | **Zorunlu** | Metin tabanlı "Round 2/3" farklı worktree'lerde tutarsızlaşır |
| "Sadece bir tur daha deneyelim" diye uzatma riski varsa | **Zorunlu** | ledger.attempt >= max_attempts → status="escalated" otomatik tetiklenir |
| Sleuth, kraken, veya uzun swarm phase | **Zorunlu** | Birden fazla agent'ın aynı task state'ini görmesi gerekir |

**Kural:** Eğer eline "Round N/3" veya "deneme 2/3" yazma ihtiyacı geliyorsa → ledger kullan. Aksi halde context loss ve race kaçınılmazdır.

### Concrete Integration Points (Orchestrator Akışında Tam Yerler)

1. **Setup (IMPL_ID belirlendikten hemen sonra)**
   ```python
   from bundled.skills.shared.task_lifecycle import TaskLifecycleLedger, make_devqa_handoff_context

   ledger = TaskLifecycleLedger(session_id=workspace_id)  # workspace_id memory.py'den gelir
   task_state = ledger.start_or_resume(
       task_id=IMPL_ID,
       objective=description,
       max_attempts=3
   )
   # task_state'i orchestrator state'e koy (self.task_state, self.ledger)
   ```

2. **Step 0 (Memory Retrieval sonrası) + Step 1 (Implement launch)**
   ```python
   context = make_devqa_handoff_context(ledger, IMPL_ID)
   # context["task_lifecycle"] ve context["handoff"] bloklarını
   # implementer prompt'una verbatim enjekte et
   ```

3. **Step 3 (Merge & Check) — her review round sonunda**
   ```python
   open_issues = [...]  # review_file'dan parse ettiğin liste
   state = ledger.record_attempt(
       IMPL_ID,
       feedback=merged_review_summary,
       issues=[i["description"] for i in open_issues],
       metadata={"round": round_count, "severity_counts": total_issues_by_severity}
   )
   if state.status == "escalated":
       # Step 3a'ya git (Escalation)
   ```

4. **Step 3a (Escalation)**
   ```python
   ledger.escalate(
       IMPL_ID,
       reason="3 attempts exhausted with open issues",
       recommendation="Present 5 options to user (Reassign/Decompose/Revise/Defer/Accept)"
   )
   ```

5. **Step 4 (Fix) ve Step 5 (Re-review) prompt'larında**
   - Her resume prompt'una `make_devqa_handoff_context(ledger, IMPL_ID)` sonucunu ekle.
   - Implementer "bu attempt'te ne düzelttim" diye yazarken ledger state'i okuyabilir (opsiyonel ama yüksek kaliteli handoff için önerilir).

### Ready-to-Paste Orchestrator Helper (Production — includes spawn_helper)

```python
# --- Task Lifecycle Ledger + Spawn Helper (Production Contract) ---
from bundled.skills.shared.task_lifecycle import TaskLifecycleLedger, make_devqa_handoff_context
from bundled.skills.shared.spawn_helper import build_spawn_context, spawn_with_discipline

if not hasattr(self, 'ledger'):
    self.ledger = TaskLifecycleLedger(session_id=workspace_id)
    self.task_state = self.ledger.start_or_resume(IMPL_ID, description, max_attempts=3)

# When preparing any spawn (implementer or reviewers)
handoff_ctx = make_devqa_handoff_context(self.ledger, IMPL_ID)

# Preferred: use the helper for automatic injection (recommended for all new code)
# ctx = build_spawn_context(
#     ledger=self.ledger,
#     task_id=IMPL_ID,
...
# Then when actually spawning (in the orchestrator code that calls the tool):
# impl_task = spawn_with_discipline(
#     spawn_subagent_fn=spawn_subagent,
#     subagent_type=...,
#     description=...,
...
# After review round
self.task_state = self.ledger.record_attempt(...)
if self.task_state.status == "escalated":
    ...
```

**Rule:** For any spawn in 2+ round flows, prefer `spawn_with_discipline` (or at minimum manually inject the result of `build_spawn_context`). This is now the Production Contract for implement.
```

### Pitfalls (Bilinen Tuzaklar)

- **Metin ile state çakışması:** "Round 2/3" diye hem ledger'a hem de review_file'a yazma. Ledger tek kaynak olsun; review_file sadece bulguları taşısın.
- **IMPL_ID vs task_id:** IMPL_ID (uuid) task_id olarak kullan. Bu, aynı workspace'teki farklı implement run'larını ayırır.
- **Windows lock:** Ledger'ın advisory lock'u (file creation) Windows'ta best-effort'tur. Yüksek eşzamanlılıkta nadir race'ler teorik olarak mümkündür — üretimde 3'ten fazla paralel /implement beklenmez.
- **Eski metin template'leri:** Handoff skill'indeki "deneme N/3" cümleleri hâlâ UX için iyidir. Onları silme; sadece arkadaki gerçek sayıyı ledger'dan al.

### Bugün (2026-06) Yapılan Değişiklik
Bu "Production Contract" bölümü, önceki "recommended" paragrafın yerini alacak şekilde eklendi. Bundan sonra implement orchestrator'ı ledger olmadan 2+ round'lu iş yapmamalıdır. Bu, the original Claude Code AI software team system'in Bounded QA-Loop disiplininin Grok-native executable karşılığıdır.

Tüm yeni bundled ajanlar (kraken, coroner, janitor, security-reviewer) bu kurala atıf yapar.
