...
### 6d: Faz 2 — Automatic Compound Analysis after Design (Proactive)

If the design involved significant architectural decisions, complexity, or trade-offs, the orchestrator **automatically executes** a lightweight compound analysis:

Use `run_terminal_cmd` to run:
```bash
python3 ~/.grok/skills/compound-learnings/scripts/analyze.py --min 2 --source design
```

Capture the stdout. If the output contains High or Medium impact proposals or meta-patterns, include a section titled **"Compound Learnings from this Design"** in the Final Report, summarizing the key findings and any suggested improvements (rules, persona updates, etc.).

This is the first real automatic cross-workflow compound learning trigger in the system.

### 6e: Faz 2 — Friction Ledger Briefing (Design Time) — Parallel to Implement/Execute-Plan

Immediately after (or in parallel with) the compound analysis above, the orchestrator also prepares a lightweight friction briefing from the ledger:

```bash
python3 -c '
from pathlib import Path
import json
ledger = Path.home() / ".grok" / "compound-friction.jsonl"
if ledger.exists():
    recent = []
    for line in list(open(ledger))[-10:]:
        try:
            r = json.loads(line)
            if r.get("impact") in ("High", "Medium"):
                recent.append(r.get("pattern",""))
        except: pass
    if recent:
        print("## Known High-Friction Patterns (from ledger)\n" + "\n".join(f"- {p}" for p in dict.fromkeys(recent)[:6]))
'
```

If non-empty output is produced, the orchestrator:
- Injects it (as a short "Known Historical Friction" block) into the **final writer revision prompt** (Step 4) so the PR Plan and Key Decisions already reflect past pain points.
- Includes the same block in the Final Report under a new subsection **"Friction Context Considered During Design"**.

This ensures that design artifacts produced by this skill carry the same friction awareness that implement and execute-plan now enforce at runtime via `friction_checklist_brief`.

Graceful degradation: if the ledger is empty or the command fails, silently skip — never block the design.
...
**Faz 2 — Proaktif Compound Learning (Design Sonrası Tetikleme):**

Büyük mimari kararlar içeren design çalışmaları, compound-learnings için yüksek kaliteli sinyal kaynağıdır.

Design tamamlandıktan sonra orchestrator şu adımı önerir / yapar:

```bash
python3 ~/.grok/skills/compound-learnings/scripts/analyze.py --min 2 --source design
```

Bu çağrı, design'daki Key Decisions ve karmaşık trade-off'lardan kaynaklanan friction pattern'leri tespit etmek için kullanılır.

Gelecekte bu adım tamamen otomatikleştirilebilir. Şimdilik Final Report'ta bu öneri açıkça yer alır.
