...
Before executing any plan on non-trivial work:

1. **Explore / Understand First** — If the design doc or plan references existing code, perform structured exploration (read key files, use tldr / architecture analysis) before spawning implementers. Never implement blind.
2. **Use Handoff Templates** — All subagent prompts (implementer + reviewer) must use structured handoffs from the `handoff` skill (`C:\Users\kaana\.grok\skills\handoff\SKILL.md`). Reference the templates explicitly in prompts for clarity.
3. **Human Checkpoint Discipline** — Major phase boundaries (plan approval, post-implementation verification) require explicit human sign-off before proceeding.

These constraints are ported from the highest-leverage patterns in the the original Claude Code AI software team system (build, explore, qa-loop, handoff-templates). They dramatically reduce context loss and escaped issues.
...
- `compound_analysis_output`: `""` -- full stdout captured from `analyze.py --min 3 --draft` after memory flush (Step 10). Embedded verbatim into the Final Report.
- `compound_drafts_generated`: `[]` -- paths to High-impact drafts generated for permanent record and later --apply.
...
- `friction_checklist_brief`: `""` -- Faz 2: Ledger'dan gelen friction pattern'lerine göre dinamik checklist (friction_checklists.py kullanılarak hazırlandı). Her PR implementer ve reviewer prompt'larına enjekte edilir.
...
### Faz 2 — Friction Checklist Hazırlama (Dinamik)

Ledger'dan yüksek friction pattern'leri okuyarak kategorilere göre dinamik checklist metni hazırla. Bu blok, implement skill'indeki aynı mantığın execute-plan için uyarlanmış halidir.

**Orchestrator olarak şu adımları GERÇEKLEŞTİR** (run_terminal_cmd ile):
        ...
        for line in f:
            try:
                ...
                if rec.get("impact") in ("High", "Medium"):
                    patterns.append(rec.get("pattern", ""))
            except:
                continue
    if patterns:
        mod_path = Path.home() / ".grok" / "skills" / "compound-learnings" / "scripts" / "friction_checklists.py"
        if mod_path.exists():
            spec = importlib.util.spec_from_file_location("friction_checklists", mod_path)
            mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(mod)
            brief = mod.get_checklist_for_ledger_patterns(patterns)
print(brief)
' 2>/dev/null || echo ""
```

Yukarıdaki komutun çıktısını `friction_checklist_brief` state değişkenine kaydet.

Eğer dosya yoksa, modül bulunamıyorsa veya hata olursa: `friction_checklist_brief = ""` (graceful degradation — run'ı bloklama).

Bu `friction_checklist_brief`, her PR'nin implementer ve reviewer prompt'larına enjekte edilir (aşağıda Step 4a ve Step 5a).
...
**Production Contract (mandatory for review-fix cycles):** Use `spawn_with_discipline` from `bundled/skills/shared/spawn_helper.py` (with per-PR ledger) for the launch. This auto-injects current ledger state + structured handoff (critical for correct attempt tracking across parallel PRs and resumptions).

**Prepend the implementer persona instructions** (loaded during setup) to the prompt.

# Faz 2 — Friction Checklist (Dinamik) — state'ten enjekte edilir
# friction_checklist_brief değişkeni Step 0'da ledger + friction_checklists.py ile hazırlandı.
# Aşağıdaki prompt'larda <friction_checklist_brief> bloğu otomatik olarak yerleştirilir.
...
**Bu bölüm, `docs/role-assignment.md` içindeki QA-Loop Disiplini ve Bounded Retry kurallarına tam uymak zorundadır.**
...
**Sıkı Sınırlama (Kesin):**
- Her PR için maksimum **3 review-fix round** yapılabilir.
- `pr.review_rounds >= 3` ve hâlâ açık issue varsa → **Stalemate / Escalation** yoluna girilir (aşağıdaki kurala bak).
...
- `pr.review_rounds` 3'e ulaşana kadar devam edilir.
- 3. round sonunda hâlâ açık issue varsa → escalation mekanizması devreye girer (insana sorulur).
...
- **Stalemate / Escalation (Faz 1 — Bounded QA-Loop):**
- Her PR için maksimum 3 review-fix round.
- 3. round sonunda hâlâ açık issue varsa → escalation tetiklenir.
- Escalation'da kullanıcıya net 5 seçenek sunulur: Reassign / Decompose / Revise / Defer / Accept.
- Kullanıcı kararı nihai kabul edilir. "Bir tur daha" uzatması yapılmaz.

Bu kural `docs/role-assignment.md` QA-Loop disiplinine tam bağlıdır.
...
This step runs **orchestrator-side** (no subagent). Execute it after the memory helper (`python3 "${MEMORY_HELPER}" update`) returns successfully in Step 10d.

**Hook-powered auto behaviors (default & automatic)**

Büyük plan run'larının sonunda kritik hook'lar artık **varsayılan olarak otomatik tetikleniyor**:
...
run_hook("on_compound_analysis_start", session_context=...)
# Swarm hooks (when execute-plan is part of larger swarm run)
# run_hook("on_phase_end", phase=..., swarm_id=...)
# run_hook("on_swarm_phase", event=..., phase=5, ...)  # verify + compound close
```

Bu sayede friction → analyzer → draft → apply + feedback döngüsü execute-plan'da da otomatik hale geliyor.
...
1. **Invoke the analyzer via shell and capture everything**
   ...
   ```
   - Use `run_terminal_cmd` (same style as the memory update call in this same step).
   - Capture **both** stdout and stderr + exit code.

2. **Store the raw output (mandatory)**
   - Set `compound_analysis_output = stdout` (full text, including any banners and draft lines).
   - If exit code != 0:
     - Prepend a clear failure header:
       "COMPOUND ANALYZER FAILED (exit {code})\n{stderr}\n--- stdout ---\n{stdout}"
     - Warn the user once.
     - Continue (this must never abort the plan execution or Final Report).

3. **Parse draft paths (best-effort, deterministic)**
   - Walk the captured stdout line by line.
   - Collect any line containing the arrow pattern "→ " that points to a path under `skills-drafts/compound-learnings/`.
   - Also harvest paths from any "Draft artifacts generated" summary block.
   - Store the unique list (in order of appearance) in `compound_drafts_generated`.

4. **Graceful degradation (identical to memory helper policy)**
   - If the analyzer cannot run (missing python, script not present, timeout, permission denied, etc.):
     `compound_analysis_output = "COMPOUND ANALYZER UNAVAILABLE: {short reason}. Self-improvement data not collected for this execution."`
     `compound_drafts_generated = []`
   - The rest of the run (stack assembly, Final Report) must still succeed.

5. **Transition to Final Report**
   - The Final Report section (point 8) is required to render `compound_analysis_output` **verbatim** under the heading "Compound Learnings & Self-Improvement Proposals".
   - When `compound_drafts_generated` is non-empty, the report must surface the paths and the one-line `--apply` instruction.

This makes the capture behavior for large plan executions identical in rigor to the `/implement` orchestrator and eliminates any ambiguity about how the self-improvement data is produced and persisted.
...
8. **Compound Learnings & Self-Improvement Proposals** (permanent record)
   - This section **always** appears in the Final Report for execute-plan runs.
   - Read the orchestrator state variables `compound_analysis_output` and `compound_drafts_generated` (populated by the Explicit Capture Protocol in Step 10d after memory flush).
   - Render `compound_analysis_output` **verbatim** (full text, banners, proposals, meta-patterns) under the heading.
   - When `compound_drafts_generated` contains paths, append a "Generated drafts" list + the exact later invocation:
     `python3 ~/.grok/skills/compound-learnings/scripts/analyze.py --apply`
   - On analyzer failure or unavailability, render the error-prefixed output anyway — the historical record must be complete and honest.
   - After the full report, if high-impact drafts were produced, offer `--apply` (the same safe per-change diff + timestamped backup flow as /implement).
   - This is the direct realization (for large multi-PR plans) of the same self-improvement discipline that /implement now has: the system not only delivers the requested work, it also proposes concrete upgrades to its own personas, orchestrators, handoff discipline, factcheck-guard, etc.

This completes the integration of the Claude compound-learnings flywheel into both primary orchestrators with fully explicit capture and rendering contracts.

---

## Task Lifecycle Ledger Entegrasyonu — Production Contract (Execute-Plan)

Execute-plan is the highest-complexity orchestrator (multi-PR DAG + parallel worktrees + per-PR bounded review-fix loops). It **must** use the Task Lifecycle Ledger for every PR that goes through review rounds.

### Mandatory Rule

For every PR that enters a review-fix cycle (Step 5), the orchestrator **must** maintain a per-PR `TaskLifecycleLedger` state (or a single ledger with PR-scoped task_ids like `"<PLAN_ID>-<pr.id>"`).

**Concrete integration points:**

1. **After Step 4b (implementation complete)** — before first review:
   ```python
   ledger = TaskLifecycleLedger(session_id=workspace_id)
   state = ledger.start_or_resume(
       task_id=f"{PLAN_ID}-{pr.id}",
       objective=pr.title,
       max_attempts=3
   )
   ```

2. **Step 5a / 5c (before launching reviewer)**:
   ```python
   ctx = make_devqa_handoff_context(ledger, f"{PLAN_ID}-{pr.id}")
   # Inject ctx["task_lifecycle"] + structured handoff into reviewer prompt
   ```

3. **After every review round (Step 5b)**:
   ```python
   ledger.record_attempt(
       f"{PLAN_ID}-{pr.id}",
       feedback=review_summary,
       issues=open_issue_descriptions
   )
   if state.status == "escalated":
       # Trigger the 5-option escalation to user (Reassign/Decompose/etc.)
   ```

This is the only way to make the "max 3 rounds per PR + automatic escalation" logic non-racy across parallel PRs and potential session compactions.

### Why This Is Critical Here

- Multiple PRs run in parallel.
- Each PR has its own independent 3-round loop.
- Without the ledger, the orchestrator can easily lose track of "this PR is on round 2" across resumes or when many PRs are in flight.
- The existing text-based `pr.review_rounds` counter is useful but secondary — the ledger is the source of truth for accumulated feedback and escalation.

All future enhancements to execute-plan (better resumption, swarm-lite extensions, etc.) should assume the ledger is present for every PR under review.
...
- After memory flush: "Running Explicit Compound Capture Protocol (analyzer + full stdout + draft paths)..." (will appear verbatim in Final Report)
...
**Spawn Helper Recommendation (added in expansion closure):**
For every PR that goes through review-fix cycles, prefer using `spawn_with_discipline` from `bundled/skills/shared/spawn_helper.py` (together with per-PR `TaskLifecycleLedger`) when launching implementers and reviewers. This ensures consistent ledger state + structured handoff injection across parallel PRs and resumption scenarios. The execute-plan orchestrator already has strong per-PR ledger setup — wiring the helper on top of it is the natural next step for full Production Contract compliance (same pattern applied to swarm).
...
- **Personas now participate in the self-improvement flywheel** -- the core personas contain an explicit "Self-Improvement Flywheel Participation" section. The Explicit Compound Capture Protocol (Step 10d) will benefit from the more generalizable signals they are now expected to produce.
- **Mandatory Structured Handoff Usage (Enforceable)** -- 
  - Every non-trivial `spawn_subagent` call (implementer veya reviewer) **must** include or reference a proper handoff template from the `handoff` skill.
  - Orchestrator, spawn yapmadan önce elindeki context'in handoff kalitesinde olduğundan emin olmakla yükümlüdür.
  - Vague veya yetersiz handoff ile launch **yasaktır**. İhlal durumunda spawn gerçekleştirme veya çok güçlü uyarı + düzeltme iste.
