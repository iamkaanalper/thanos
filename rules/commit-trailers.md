# Commit Trailers — Structured Decision Context (Grok Port)
...
**Grok Note:** Use with our safety-and-quality.md commit format (`<type>: <description>`). Trailers go AFTER the body, separated by blank line. Enforced in pre-commit / verifier flows where possible. Ties into claim-verification (evidence for "why") and Production Contract (document decisions).
...
| `Scope-risk`  | Change touches shared infra, middleware, auth, or core data paths | Isolated, single-purpose change     |
...
### Constraint
An external limitation — browser API, hosting restriction, library limitation, business rule, or environment constraint — that ruled out the obvious solution.
...
### Rejected
An alternative that was seriously considered. Helps future maintainers understand why the current approach exists instead of the simpler-looking one they are thinking of.
...
- This format supplements, not replaces, the conventional commit type prefix (see safety-and-quality.md)
...
- Trailers are the commit-level equivalent of Priority Notes in the notepad system and support claim-verification (why we did it this way).
- Use in verifier / shipper flows before commit.

(Original from Claude/Thanos (Grok port of the original Claude Code AI software team system) commit-trailers; ported with Grok commit format, safety-and-quality integration, and emphasis on evidence for future agents/maintainers.)
