# Research Confidence Threshold (Grok Port)
...
**Grok Adaptation:** This rule is enforced via our claim-verification / factcheck-guard skill (two-pass: hypothesize → read actual files to confirm; never grep-only claims). Use explore subagent + tldr-cli for efficient research. Aligns with preflight + Production Contract.
...
| Confidence | Action |
|------------|--------|
| Below 70% | **Expand search scope.** You are missing fundamental understanding. Read more files, check imports, trace call chains. Use `tldr structure`, `tldr calls`, or `tldr arch` (or explore subagent) to get a broader view. |
...
2. Check for existing patterns (search for similar implementations; use tldr or grep)
3. Identify the test file and understand test conventions
4. Assess your confidence against the four questions above
5. If 90% or above, start writing code (or spawn implementer)
6. If below, identify exactly what you do not know and target that gap

**Grok Note:** Before any existence/behavior claim in audits or plans, apply factcheck-guard (two-pass). Pre-flight often includes this discipline. See claim-verification.md and factcheck-guard skill.

(Original from Claude/Thanos (Grok port of the original Claude Code AI software team system); ported with Grok executable + claim-verif emphasis. No behavior change to the threshold itself.)
