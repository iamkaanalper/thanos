...
description: Graceful Degradation (Grok Adapter). Grok-native port/adaptation. This skill provides the Graceful Degradation capability. In Grok, many specialized skills are implemented via high-leverage patterns + meta skills + general-purpose agents. See bundled/skills and assignment matrix.
when-to-use: When the task involves Graceful Degradation concepts, patterns, or workflows. Consult the agent-assignment-matrix for exact routing.
---

# Graceful Degradation (Grok Adapter)
...
## Role & Purpose
Graceful Degradation patterns, best practices, and workflows.
