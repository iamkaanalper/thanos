...
1. If the question is about **current config** (what MCP servers, models, or settings are active),
   read `\\?\C:\Users\kaana\.grok/config.toml`. MCP servers are under `[mcp_servers.*]` sections.
...
2. If the question is about **how to do something** (setup, adding MCP servers, creating skills,
   ...
   `\\?\C:\Users\kaana\.grok/docs/user-guide/`. The available guides are:
   ...
   `\\?\C:\Users\kaana\.grok/README.md` for the comprehensive reference.

3. To **modify config** for the user, edit `\\?\C:\Users\kaana\.grok/config.toml` with search_replace.

4. To **create a skill** for the user, create `\\?\C:\Users\kaana\.grok/skills/<name>/SKILL.md`
   (read `\\?\C:\Users\kaana\.grok/docs/user-guide/08-skills.md` for the SKILL.md format).